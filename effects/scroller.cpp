...
void Scroller::draw() {
    ...
    for(int i=0; i<text.length(); i++) {
        ...
        float y = fmod((1.0f+length)-i*C_DIST+common->t*0.3f, 1.0f+length)-length;
        mvp.setModel(sin(y*4.5+M_PI)*0.5 + sin(y*9.0+common->t*2)*0.08, y, 0.0f, y*2.4, y*2.0, y, C_SCALE+sin(y*15.0)*0.017+0.017);
        //mvp.buildModel();
        mvp.buildMVP();
